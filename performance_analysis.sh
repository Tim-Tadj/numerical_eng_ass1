...
do
    ...
    if [ $program == "mb5.out" ]; then
        echo "Running $program with $iterations iterations, x origin = $x, $y y, $size size"
        /usr/bin/time ./$program $iterations $x $y $size
    # the fork program takes an extra argument specifying socket or pipe
    # given as 's' and 'p' respectively
    elif [ $program == "mb5_fork.out" ]; then
        echo "Running $program with $iterations iterations, $x x, $y y, $size size, $threads threads, $ipc ipc"
        /usr/bin/time ./$program $iterations $x $y $size $threads $ipc
    else
        echo "Running $program with $iterations iterations, $x x, $y y, $size size, $threads threads"
        /usr/bin/time ./$program $iterations $x $y $size $threads
    fi
